...
from schedule.forms import tt1, UserLoginForm
...
def timetable(request):
	if 'q' in request.GET:
		q=request.GET['q']
		multiple_q=Q(Q(fna1__fname__icontains=q)|Q(ex__exam_date__icontains=q)|Q(ex__subject__icontains=q))
		data=conduct.objects.select_related('ex','fna1','room').filter(multiple_q)
	else:
		data=conduct.objects.select_related('ex','fna1','room').all()
...
def assignfac(request,exid):
	if request.method=="POST":
		try:
			...
			# if x.fname!=f:
			data=conduct.objects.create(fna1=x,ex=exobj,room=r)
			if data:
				...
				# s3=conduct.objects.filter(ex=d1).values('fna2')
				# dz=dy.exclude(fname__in=s3)
				return render(request,'schedule/assignfac.html',{'x':d1,'z':data3})
		except Exception:
			messages.warning(request,'Please enter  valid details!!!.......')
			d=exam.objects.all()
			return render(request,'schedule/adminpage.html',{'data':d})
...
def facstatus(request):
	...
	y=tt.objects.filter(Branch='ISE')
...
def timetable5(request,fid):
	...
	c=conduct.objects.select_related('ex','fna1','room').filter(fna1=n).count()

	data2=conduct.objects.select_related('ex','fna1','room').filter(fna2=n.fname)
	c1=conduct.objects.select_related('ex','fna1','room').filter(fna2=n.fname).count()
	

	hx=head.objects.first()
	return render(request,'schedule/timetable5.html',{'data1':data1,'data2':data2,'h':hx,'c':c,})
...
def charts(request):
	...
	n=conduct.objects.all().values('room').annotate(total=Count('id'))
	return render(request,'schedule/charts.html',{'res':res,'n':n})

def stats(request):
	res=conduct.objects.all().values('fna1').annotate(total=Count('id'))
	return render(request,'schedule/stats.html',{'res':res})
